...
void Encode(void)
{
	...
	unsigned int real_textsize;
	
	fseek(infile, 0L, SEEK_END);
	textsize = ftell(infile);
	real_textsize = (unsigned int)textsize;
	if (fwrite(&real_textsize, sizeof real_textsize, 1, outfile) < 1)
		Error("Write Error");  /* output size of text */
	codesize += sizeof real_textsize;
...
void Decode(void)
{
	...
	if (fread(&real_textsize, sizeof real_textsize, 1, infile) < 1)
		Error("Read Error");  /* read size of text */
	textsize = real_textsize;
	if (textsize == 0) return;
